"""End-to-end checks for one session-scoped `fetch_rates()` fan-out."""
...
def test_houses_are_fetched_concurrently(fanout):
    # Sequential fetching of this many houses would take far longer; the bound
    # is loose enough to tolerate a slow house but tight enough to catch the
    # fan-out silently becoming serial.
