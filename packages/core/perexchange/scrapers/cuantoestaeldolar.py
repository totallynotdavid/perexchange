...
# A plain GET returns the exchange-house data in a Next.js React Server Components
# flight payload, not in rendered markup. Decode the `self.__next_f.push(...)` rows
# directly because the DOM is populated only after client hydration.
...
def _find_houses(html_content: str) -> list[dict[str, Any]] | None:
    # A page can embed preview and full-list rows. The complete listing is the
    # longest candidate in the current response shape.
...
def _house_to_rate(house: dict[str, Any]) -> ExchangeRate | None:
    # status 1 means the house is currently shown on the page; inactive/hidden
    # listings keep stale or blank rate fields.
...
def _parse_html(html_content: str) -> list[ExchangeRate]:
    ...
    # Do not re-emit a rate already covered by a dedicated adapter.
