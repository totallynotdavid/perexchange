...
def _parse_json(data: dict[str, Any]) -> list[ExchangeRate]:
    # The order book has no stable quote. Use adminOpenData, which is the published
    # reference shown by cuantoestaeldolar. The start value is the sale price; the
    # detraction value is the seller's net amount and maps to `buy_price`.
    try:
